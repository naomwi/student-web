            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="space-y-1">
                  <Link href="/fptcolearn/exams" className={`block text-sm px-3 py-2 rounded-lg transition ${!yearFilter ? 'bg-[#0D9488]/10 text-[#0D9488] font-medium' : 'hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                    Tất cả
                  </Link>
                  {years.map(y => (
                    <Link key={y} href={`/fptcolearn/exams?year=${y}`} className={`block text-sm px-3 py-2 rounded-lg transition ${yearFilter === y ? 'bg-[#0D9488]/10 text-[#0D9488] font-medium' : 'hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                      {y}
                    </Link>
                  ))}
                </div>
